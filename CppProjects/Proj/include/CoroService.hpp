...
namespace Bicycle
{
	...
	namespace CoroService
	{
		...
		// Класс абстрактного дескриптора, использующего сервис сопрограмм
		class BasicDescriptor;

		typedef std::pair<BasicDescriptor*, SpinLock> PtrWithLocker;
		...
		class Service
		{
			...
			private:
				...
				Error Post( const std::function<void()> &task );
		...
		class BasicDescriptor
		{
			...
			protected:
				...
				/// Добавление задачи в Post сервиса
				Error PostToSrv( const std::function<void()> &task );
